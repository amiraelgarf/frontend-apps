...
function band() {
  var scale = transform.ordinal().unknown(undefined),
      domain = scale.domain,
  ...
  return transform.initRange.apply(rescale(), arguments);
}
...
const D3BarChart = class {
    ...
    componentDidLoad() {
        this.svg = transform.select(this.hostElement.shadowRoot.querySelector('svg'));
    ...
    drawChart = () => {
        ...
        const yScale = linear.linear()
            .domain([0, linear.max(this.chartData, d => d.value) || 0])
            .range([innerHeight, 0]);
        g.append('g')
            .attr('class', 'x-axis')
            .attr('transform', `translate(0,${innerHeight})`)
            .call(linear.axisBottom(xScale))
            .selectAll('text')
            .attr('class', 'text-sm fill-gray-700');
        g.append('g')
            .attr('class', 'y-axis')
            .call(linear.axisLeft(yScale))
            .selectAll('text')
            .attr('class', 'text-sm fill-gray-700');
        g.selectAll('.bar')
            .data(this.chartData)
    ...
    render() {
        return (index.h("div", { key: '7479f4035fdd8ab4a1b1469db98c6b4e3b69411b', class: "chart-container" }, index.h("svg", { key: '7293508145235de53bb2f2310882796c698b3f4e' })));
    }
    static get watchers() { return {
        "chartData": ["onDataChange"]
    }; }
};
D3BarChart.style = d3BarChartCss;

exports.d3_bar_chart = D3BarChart;
//# sourceMappingURL=d3-bar-chart.entry.cjs.js.map

//# sourceMappingURL=d3-bar-chart.cjs.entry.js.map
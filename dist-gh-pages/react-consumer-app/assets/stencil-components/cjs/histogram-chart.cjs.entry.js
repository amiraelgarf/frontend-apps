...
function nice(start, stop, count) {
  let prestep;
  while (true) {
    const step = linear.tickIncrement(start, stop, count);
...
function bin() {
  ...
  function histogram(data) {
    ...
    if (!Array.isArray(tz)) {
      ...
      tz = linear.ticks(x0, x1, tn);

      // If the domain is aligned with the first tick (which it will by
      // default), then we can use quantization rather than bisection to bin
      // values, which is substantially faster.
      if (tz[0] <= x0) step = linear.tickIncrement(x0, x1, tn);
      ...
      if (tz[tz.length - 1] >= x1) {
        if (max >= x1 && domain === extent) {
          const step = linear.tickIncrement(x0, x1, tn);
          if (isFinite(step)) {
            if (step > 0) {
    ...
    if (isFinite(step)) {
      ...
      for (i = 0; i < n; ++i) {
        if ((x = values[i]) != null && x0 <= x && x <= x1) {
          bins[linear.bisectRight(tz, x, 0, m)].push(data[i]);
        }
      }
    }
...
const HistogramChart = class {
    ...
    renderHistogram() {
        ...
        const svg = transform.select(this.el.shadowRoot?.querySelector('.histogramChart'))
            .append('svg')
        ...
        const x = linear.linear()
            .domain([0, linear.max(allGrade) || 0])
            .range([0, width])
            .nice();
        const bins = bin()
            .value(d => d)
        ...
        const y = linear.linear()
            .domain([0, linear.max([...semester1Bins, ...semester2Bins], d => d.length) || 0])
            .range([height, 0])
            .nice();
        const color = transform.ordinal()
    ...
    render() {
        return (index.h("div", { key: 'cc839c4bb2873fae2b2da47a9afe8e753348781c', class: "histogramChart" }));
    }
...
exports.histogram_chart = HistogramChart;
//# sourceMappingURL=histogram-chart.entry.cjs.js.map

//# sourceMappingURL=histogram-chart.cjs.entry.js.map